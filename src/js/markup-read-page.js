...
function createMarkupAlreadyReadNews() {
  ...
  const markupWrapper = uniqueDates.map((date) => {
    return `<div class="read__dropdown-wrapper">
            <h2
              class="read__date-title"
              data-date="${date}"
            >${date}
            <svg class="read__date-icon-down">
              <use href="${svg}#icon-arrow-down"></use>
            </svg>
            <svg class="read__date-icon-up">
  ...
  const correspondingBtn = document.querySelector(`.read__date-title[data-date="${date}"]`)
