...
function createMarkupAlreadyReadNews() {
  ...
  const filteredNews = alreadyReadArray.filter(item => item.dateOfRead === date)
  const markupNewsForDate = filteredNews.map(({ uri, src, alt, title, subtitle, date, href, category, dateOfRead }) => {
    return `<li
  ...
  const correspondingBtn = document.querySelector(`.read__dropdown-content-admin-btn[data-date="${date}"]`)
  const contentList = correspondingBtn.nextElementSibling.querySelector('.read__dropdown-content-list')
  contentList.innerHTML = markupNewsForDate
})
  ...
  // readDropdownContentAdminBtn.forEach(btn => {
  //   const btnDateAttribute = btn.getAttribute('data-date')

  //   alreadyReadArray.forEach(({ dateOfRead }) => {
  //     if (btnDateAttribute === dateOfRead) {
